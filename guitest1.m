...
% Last Modified by GUIDE v2.5 03-Aug-2018 15:00:07
...
function guitest1_OpeningFcn(hObject, ~, handles, varargin)
...
function varargout = guitest1_OutputFcn(~, ~, handles) 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
im = imread('cameraman.tif');   % 读图
axes(handles.axes1)          % 当前画图轴
imshow(im);                  % 显示图像
%axes off；
%创建imscrollpanel对象
k = get(hObject,'value');        % 滑动条值
imshow(k.*im)                % 显示图像
